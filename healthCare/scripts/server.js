...
app.post("/GetPatientData", (req, res) => {
  ...
  const patientQuery = `SELECT * FROM patient WHERE ${demographic} = '${value}';`;
...
app.post("/createaccount", async (req, res) => {
  const { username, password, accountType, assignedRegion, accessLevel } = req.body;
  ...
  con.query(userquery, [username], async (error, results) => {
    ...
    try {
      ...
      con.query(addquery, [username, hashedPassword, accountType, assignedRegion, accessLevel], (error, results) => {
        if (error) {
...
app.post("/addPatientSymptoms", (req, res) => {
  ...
  const checkuser = `SELECT * FROM accounts WHERE natID = ?`;
...
app.post("/addPatientExam", (req, res) => {
  ...
  //make sure the username is unique - if not, return
  const checkuser = `SELECT * FROM patientInfo WHERE natID = ?`;
...
app.get("/getAggregatedData", (req, res) => {
  ...
  con.query(query, (error, results) => {
    ...
    res.status(200).json(results.rows); //return aggregated data
  });
});
